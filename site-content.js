(() => {
  ...
  const find = selector => { try { return selector ? document.querySelector(selector) : null; } catch { return null; } };
  ...
  const applyStyles = (element, styles) => Object.entries(styles || {}).forEach(([property, value]) => { if (!allowedStyle.has(property) || value === '' || value == null) return; element.style.setProperty(cssName(property), String(value)); });

  const resolveDraftElement = draft => {
    let element = find(draft.selector);
    if (!element && draft.fallbackSelector) {
      element = find(draft.fallbackSelector);
      if (element && draft.stableId) element.dataset.mohajerEditorId = String(draft.stableId);
    }
    return element;
  };
  ...
  const applyPublishedContent = drafts => Object.values(drafts || {}).forEach(draft => {
    ...
    else if (['text','link','heading','paragraph','button'].includes(draft.type)) setSafeText(element, draft.content);
    if (Number(draft.schemaVersion || 0) >= 6) {
      applyStyles(element, draft.styles);
      applyStyles(element, draft.responsive?.desktop || {});
    } else {
  ...
  const sanitizeTree = element => {
    ...
    Array.from(element.attributes).forEach(attribute => {
      ...
      if (name.startsWith('on') || name === 'srcdoc' || (name === 'href' && !safeUrl(attribute.value)) || (name === 'src' && !safeUrl(attribute.value))) element.removeAttribute(attribute.name);
    });
  ...
  const applyStructure = operations => (operations || []).forEach(operation => {
    ...
    if (operation.action === 'delete') {
      const element = document.querySelector(marker) || find(operation.selector) || find(operation.fallbackSelector);
      element?.remove();
      return;
    }
    if (operation.action === 'move') {
      const element = find(operation.selector) || find(operation.fallbackSelector);
      const parent = find(operation.parentSelector) || find(operation.parentFallbackSelector);
      const before = find(operation.beforeSelector);
    ...
    if (operation.action === 'add' && operation.html && !document.querySelector(marker)) {
      const parent = find(operation.parentSelector) || find(operation.parentFallbackSelector) || document.body;
  ...
  const loadPublishedContent = () => app.firestore().collection('siteContent').doc('published').get()
    .then(snapshot => {
      ...
      // Content first establishes stable editor IDs; structural operations can then
      // safely reference those IDs or their original fallback selectors.
    ...
    .finally(() => { window.clearTimeout(revealTimeout); revealSite(); });

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', loadPublishedContent, { once:true });
  else loadPublishedContent();
})();
