(() => {
  ...
  const addCustomCss=(e,d)=>{if(!d.customCss||!d.stableId)return;const id='mohajer-published-css-'+String(d.stableId).replace(/[^a-zA-Z0-9_-]/g,'');let s=document.getElementById(id);if(!s){s=document.createElement('style');s.id=id;document.head.appendChild(s)}const sid=String(d.stableId).replace(/[^a-zA-Z0-9_-]/g,'');s.textContent=`[data-mohajer-editor-id="${sid}"]{${String(d.customCss)}}`};
  const applyHover=(e,d)=>{if(!d.hover?.scale||d.hover.scale==='1')return;e.style.setProperty('--mohajer-hover-scale',d.hover.scale);if(!document.getElementById('mohajer-hover-style')){const s=document.createElement('style');s.id='mohajer-hover-style';s.textContent='[data-mohajer-editor-id]{transition:transform .25s ease} [data-mohajer-editor-id]:hover{transform:scale(var(--mohajer-hover-scale,1))}';document.head.appendChild(s)}};
  const applyDrafts=drafts=>Object.values(drafts||{}).forEach(d=>{if(!d?.selector)return;const e=resolve(d);if(!e)return;if(d.type==='image'&&validImage(d.content))e.src=d.content;else if(['text','link','heading','paragraph','button'].includes(d.type))setText(e,d.content);applyStyles(e,d.styles);applyStyles(e,d.responsive?.desktop||{});if(d.hidden)e.style.setProperty('display','none','important');if(d.alt&&e.tagName==='IMG')e.alt=d.alt;if(d.link&&(e.tagName==='A'||e.closest('a'))&&safeUrl(d.link)){const a=e.tagName==='A'?e:e.closest('a');a.href=d.link;if(d.newTab)a.target='_blank';else a.removeAttribute('target')}applyHover(e,d);addCustomCss(e,d)});
